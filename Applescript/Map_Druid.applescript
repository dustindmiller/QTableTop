tell application id "com.figure53.QLab.4" to tell front workspace
	...
	display dialog "Create or Modify?" with title "Map Druid" with icon 1 buttons {"Create", "Modify", "Cancel"} default button "Create" cancel button "Cancel"
	set actionType to button returned of result
	if actionType is "cancel" then
		return

	else if actionType is "Modify" then

	set selectedArea to (choose from list areaList with title "Map Druid" with prompt "Select Area of Map to Modify..." default items "None" OK button name {"Select"} cancel button name {"Cancel"}) as string
	if selectedArea is "false" then return

	set mapNameList to (q number of cues of cue selectedArea)
			set newTarg to (choose from list mapNameList with title "Map Druid" with prompt "Select Map to Modify..." default items "None" OK button name {"Modify"} cancel button name {"Cancel"}) as string
			if newTarg is "false" then return

		set floorExists to exists of cue (newTarg & " FLOORS")
		if floorExists is true then
			set floorList to cues of cue (newTarg & " FLOORS")
			set newTarg to q number of (first item of floorList)
		end if

		set maskExists to exists cue (newTarg & " MASKS")

		set commandList to (display dialog "Select Task" with title "Map Druid" buttons {"Map Position Fix", "Manual Move"} default button "Map Position Fix" giving up after 5)
			set myChoice to button returned of commandList as string

		if myChoice is "Map Position Fix" then

				set newPosX to translation x of cue (newTarg & " BASE")
				set newPosY to translation y of cue (newTarg & " BASE")

				-- if floorExists is true then
				-- 	set moveList to q number of (cues of cue (newTarg & " MASKS"))
				-- 	set end of moveList to q number of cues of cue ((q number of second item of floorList) & " MASKS")
				-- else
					set moveList to q number of (cues of cue (newTarg & " MASKS"))

					repeat with message in moveList
						set translation x of cue message to newPosX
						set translation y of cue message to newPosY
					end repeat
				-- end if

		else if myChoice is "Manual Move"
			set selected to cue (newTarg & " BASE")
			return
		end if
