tell application id "com.figure53.QLab.4" to tell front workspace
	set contentName to "Table"
	set mapNameList to (q number of cues of cue (contentName & "s"))
	set activeMap to q name of cue (contentName & " Toggle")
	set newTarg to (choose from list mapNameList with title "Toggle Table" with prompt "Select Table..." default items activeMap OK button name {"Toggle"} cancel button name {"Cancel"}) as string
	...
	if cue newTarg is running then
		...
		set q color of cue (contentName & " Toggle") to "rufous"
