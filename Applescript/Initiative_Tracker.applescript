	...
	if cue "ALL PARTY" is not running and cue "ALL NPCs" is not running then
		...
		if doubleCheck is "Cancel" or doubleCheck is "" then
			...
			repeat
				...
				set newTarg to (choose from list partyList & "——————————" & npcList with title "Set Initiative Order" with prompt "Select Token..." default items "None" OK button name {"Select"} cancel button name {"Cancel | Finish"}) as string
